...
#define MAXTSIZE   20000       /* max size for raw time series */
...
usinfo ()
	...
	if (InfoLimits)
		{
		plim (Argv0);
		const (MAXTSIZE, "maximum size of input time series");
		const (MAXCHARS, "maximum number of characters in input fields");
		}
	if (InfoOptions)
		{
		ppgm (Argv0, Purpose);
