...
#	$BIN/spot-price-aggregate -f $FILES | sort -n -k 1 -k 2 | uniq | tail -n $MAXSIZE > $TARG/$REGION-$TYPE-agg.txt
	...
	$BIN/bmbp_ts -f $TARG/$REGION-$TYPE-agg.txt -T -i 350 -q 0.975 -c 0.01 | grep "pred:" | awk '{print $2,$4,($6+0.0001),$14}' > $TARG/$REGION-$TYPE-pred.txt
	...
	$BIN/pred-distribution -f $TARG/$REGION-$TYPE-temp.txt -q 0.025 -c 0.99 -F 4.0 -I 0.05 | awk '{print $1/3600,$2}' > $TARG/$REGION-$TYPE.pgraph
