...
echo "<h2>Spot Price Predictions by AWS Region</h2>"
echo "<h3>$NOW</h3>"
...
for REGION in `cat $HERE/regions2.txt` ; do
	...
	for INSTTYPE in `cat $HERE/instances.txt` ; do
		...
		$HERE/make-plotly.sh $REGION $INSTTYPE > $HERE/../html/$REGION-$INSTTYPE.html
		chmod 644 $HERE/../html/$REGION-$INSTTYPE.html
		if ( ! test -z "$EVEN" ) ; then
			echo '<tr class="even">'
			EVEN=""
		else
			...
			echo "<td><a href=http://128.111.84.183/$REGION-$INSTTYPE.html>What Should I Bid?</a></td>"
		echo "</tr>"
	done
	echo "</tbody>"
	echo "</table>"
